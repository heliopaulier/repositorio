# ia/ia_helper.py
# -*- coding: utf-8 -*-
# OCR e heurísticas que serão invocadas pelo Sikuli (Jython) via subprocess
...
def ocr_image(path_image: str, lang: str = 'por') -> str:
    """Faz OCR da imagem e retorna o texto detectado."""
...
def detect_popup_by_ratio(path_image: str) -> str:
    """Heurística simples: se uma grande área for branca, possivelmente é popup."""
    img = cv2.imdecode(np.fromfile(path_image, dtype=np.uint8), cv2.IMREAD_COLOR)
    if img is None:
        return 'NORMAL'
    h, w = img.shape[:2]
...
if __name__ == '__main__':
    # CLI: python ia_helper.py ocr <image> | popup <image>
    if len(sys.argv) < 3:
        print('ERR')
        sys.exit(1)
    cmd = sys.argv[1]
